...
% ---------------------- USER SETTINGS ---------------------------
csv_folder_path = "C:\Users\Tirza\Documents\Abaqus_4layers\";                     % path to folder where csv files are stored
surface_names = ["vertical_x0", "horizontal_y8", "horizontal_y9", "horizontal_y10"];                  % define names of surface which you also used in the python file
all_fieldoutput = ["NT11", "S11", "S22", "S33", "Smises"];     % list all field output values which you want to plot
step_names = {'HotplateSS', 'Deposition', 'Cooling1', 'ReleaseFixtures', 'Cooling2'};
...
for plane=1:length(surface_names)
    ...
    for field_output = 1:length(all_fieldoutput)  
        ...
        videoFile = all_fieldoutput(field_output) + "_over time_" + surface_name + "surface_4layers.mp4";        % video file name
