...
# Folder in which the input files should be stored. Change this if you place this code in a different folder
os.chdir(r"C:\Users\Tirza\Documents\Abq_XS1_10trk")
...
Run_step_2 = 1                      # 1 if preheating step should be included, 0 if not
...
# Tirza Peeters change 11-11-2025 delete variable PartitionThick = depTrkThick
# Tirza Peeters change 11-11-2025 delete variable NumPartition = depThick/depTrkThick
...
# Tirza Peeters change 01-16-2026 create/rename variables for the deposition volume size (the dimensions of one block) 
depVolThick     = depTrkThick                   # Thickness of each deposition volume
depVolWidth     = depTrkWidth                   # Width of each deposition volume
depVolLength    = depVolWidth                   # Length of each deposition volume
...
# Tirza Peeters change 11-11-2025 delete variable GeneralSize = depTrkWidth*10

# Tirza Peeters change 11-21-2025 rename and create variables to control mesh size and remove hardcoding    \\\ start

# To change the mesh size, the parameters minMeshSizeX, minMeshSizeY and minMeshSizeZ are of importance.
# With minMeshSizeX the mesh size in the x-direction can be adjusted
# With minMeshSizeY the mesh size in the y-direction can be adjusted
# With minMeshSizeZ the mesh size in the z-direction can be adjusted
# These parameters also directly control the mesh size in a deposition volume
# Only change partition (not the other parameters) if you would like to have an equal amount of mesh elements along each direction in a depostion volume

partition = 0.3333333333333333          # choose this value but make sure that an integer amount fits into 1, so for example: 0.5, 0.3333333333333333, 0.25, 0.2

minMeshSizeX 	    = partition*depVolWidth
maxMeshSizeX	    = minMeshSizeX*BiasMeshSubDir1Ratio # TruWidth

minMeshSizeY        = partition*depVolThick
maxMeshSizeY	    = minMeshSizeY*BiasMeshSubDir2Ratio
maxMeshSizeYBase    = minMeshSizeY*8

minMeshSizeZ        = partition*depVolLength
minMeshSizeZBase    = minMeshSizeZ*5
maxMeshSizeZ 	    = minMeshSizeZ*BiasMeshSubDir3Ratio
# Tirza Peeters change 11-21-2025 rename and create variables to control mesh size and remove hardcoding    \\\ end

# Tirza Peeters change 11-11-2025 delete variable minSize1B = minMeshSizeX*5
# Tirza Peeters change 11-11-2025 delete variable maxSize1B	= maxMeshSizeX*5
...
for i in range(1,int(depNoOfLyrs)):
    myPart.DatumPlaneByPrincipalPlane(offset=baseThick+depTrkThick*i, principalPlane=XZPLANE) #
...
#Partitions
#BasewithDep
for i in range(int(depNoOfLyrs)-1):
    myPart.PartitionCellByDatumPlane(cells= myPart.cells.findAt(((0, baseThick+0.5*depTrkThick+i*depTrkThick, dep1Length/2), ),
    ((0, baseThick+0.5*depTrkThick+i*depTrkThick, baseLength-dep3Length/2), ), ), datumPlane=myPart.datums[i+6])

if int(depNoOfLyrs)==1:
    i=-1
...
for i in range(1,int(depNoOfLyrs)):
    myPart.DatumPlaneByPrincipalPlane(offset=baseThick+subsThick+depTrkThick*i, principalPlane=XZPLANE) #
...
for i in range(int(depNoOfLyrs)-1):
    myPart.PartitionCellByDatumPlane(cells=myPart.cells.findAt(((0, baseThick+subsThick+0.5*depTrkThick+i*depTrkThick, dep2Length/2+dep1Length), )), 
...
# Tirza Peeters change 11-24-2025 added variables for meshing vertical edges deposition /// start
dep2_y_top = baseThick + subsThick + dep2Thick
dep1_y_top = baseThick + dep1Thick
dep3_y_top = baseThick + dep3Thick

dep2_y_bot = baseThick + subsThick
dep1_y_bot = baseThick
dep3_y_bot = baseThick

# Tolerance for selecting a thin region around edges
tol = 1e-6
# Tirza Peeters change 11-24-2025 added variables for meshing vertical edges deposition /// end
...
#General Seeding
myPart.seedPart(deviationFactor=0.1, minSizeFactor=0.1, size=minMeshSizeZ)
...
myPart.seedEdgeBySize(constraint=FINER, deviationFactor=0.1, edges=selected_edges, size=minMeshSizeZ)
...
for i in range(int(depNoOfLyrs)+1):
    myPart.seedEdgeBySize(constraint=FINER, deviationFactor=0.1, edges=
        ...
        ), minSizeFactor=0.1, size=minMeshSizeX)
...
myPart.seedEdgeByBias(biasMethod=SINGLE, 
    ...
    maxSize=maxMeshSizeX, minSize=minMeshSizeX)
...
myPart.seedEdgeByBias(biasMethod=SINGLE, 
    ...
    maxSize=maxMeshSizeY, minSize=minMeshSizeY)
...
myPart.seedEdgeBySize(constraint=FINER, 
    ...
    ), minSizeFactor=0.1, size=maxMeshSizeX)   
...
# Tirza Peeters change 11-24-2025 add meshing vertical edges deposition /// start
x_positions_dep2 = [-dep2Width/2, dep2Width/2]
z_positions_dep2 = [baseLength - ((baseLength - dep2Length)/2), (baseLength - dep2Length)/2]

for x in x_positions_dep2:
    for z in z_positions_dep2:
        found_edges = myPart.edges.getByBoundingBox(
            xMin=x - tol, xMax=x + tol,
            yMin=dep2_y_bot, yMax=dep2_y_top,
            zMin=z - tol, zMax=z + tol
        )

        if len(found_edges) == 0:
            print(" No edges found at x =", x, "z =", z)
            continue

        myPart.seedEdgeBySize(constraint=FINER, deviationFactor=0.1, edges=found_edges, size=minMeshSizeY)
# Tirza Peeters change 11-24-2025 add meshing vertical edges deposition /// end
...
myPart=myModel.parts['BasewithDep']
#General Seeding
myPart.seedPart(deviationFactor=0.1, minSizeFactor=0.1, size=minMeshSizeZBase)
# Define the coordinates of the bounding box (xMin, yMin, zMin, xMax, yMax, zMax)
...
# Use the selected edges in the seedEdgeBySize method
myPart.seedEdgeBySize(constraint=FINER, deviationFactor=0.1, edges=selected_edges, size=minMeshSizeZBase)


# +X Direction
myPart.seedEdgeByBias(biasMethod=SINGLE, 
    constraint=FINER, end1Edges= myPart.edges.findAt(((dep1Width/2+(baseWidth-dep1Width)*0.5*0.1, baseThick, 0.0), )), maxSize=maxMeshSizeX, minSize=minMeshSizeX)
        
myPart.seedEdgeByBias(biasMethod=SINGLE, 
    constraint=FINER, end2Edges= myPart.edges.findAt(((dep1Width/2+(baseWidth-dep1Width)*0.5*0.1, 0, 0.0), )), maxSize=maxMeshSizeX, minSize=minMeshSizeX)
    
myPart.seedEdgeByBias(biasMethod=SINGLE, 
    constraint=FINER, end2Edges= myPart.edges.findAt(((dep1Width/2+(baseWidth-dep1Width)*0.5*0.1, baseThick, dep1Length), )), maxSize=maxMeshSizeX, minSize=minMeshSizeX)
    
myPart.seedEdgeByBias(biasMethod=SINGLE, 
    constraint=FINER, end1Edges= myPart.edges.findAt(((dep1Width/2+(baseWidth-dep1Width)*0.5*0.1, 0, dep1Length), )), maxSize=maxMeshSizeX, minSize=minMeshSizeX)
    
myPart.seedEdgeByBias(biasMethod=SINGLE, 
    constraint=FINER, end2Edges= myPart.edges.findAt(((dep1Width/2+(baseWidth-dep1Width)*0.5*0.1, baseThick, dep1Length+dep2Length), )), maxSize=maxMeshSizeX, minSize=minMeshSizeX)
    
myPart.seedEdgeByBias(biasMethod=SINGLE, 
    constraint=FINER, end1Edges= myPart.edges.findAt(((dep1Width/2+(baseWidth-dep1Width)*0.5*0.1, 0, dep1Length+dep2Length), )), maxSize=maxMeshSizeX, minSize=minMeshSizeX)
    
myPart.seedEdgeByBias(biasMethod=SINGLE, 
    constraint=FINER, end1Edges= myPart.edges.findAt(((dep1Width/2+(baseWidth-dep1Width)*0.5*0.1, baseThick, baseLength), )), maxSize=maxMeshSizeX, minSize=minMeshSizeX)
    
myPart.seedEdgeByBias(biasMethod=SINGLE, 
    constraint=FINER, end2Edges= myPart.edges.findAt(((dep1Width/2+(baseWidth-dep1Width)*0.5*0.1, 0, baseLength), )), maxSize=maxMeshSizeX, minSize=minMeshSizeX)
...
# -X Direction
myPart.seedEdgeByBias(biasMethod=SINGLE, 
    constraint=FINER, end2Edges= myPart.edges.findAt(((-(dep1Width/2+(baseWidth-dep1Width)*0.5*0.1), baseThick, 0.0), )), maxSize=maxMeshSizeX, minSize=minMeshSizeX)
    
myPart.seedEdgeByBias(biasMethod=SINGLE, 
    constraint=FINER, end1Edges= myPart.edges.findAt(((-(dep1Width/2+(baseWidth-dep1Width)*0.5*0.1), 0, 0.0), )), maxSize=maxMeshSizeX, minSize=minMeshSizeX)
    
myPart.seedEdgeByBias(biasMethod=SINGLE, 
    constraint=FINER, end1Edges= myPart.edges.findAt(((-(dep1Width/2+(baseWidth-dep1Width)*0.5*0.1), baseThick, dep1Length), )), maxSize=maxMeshSizeX, minSize=minMeshSizeX)
    
myPart.seedEdgeByBias(biasMethod=SINGLE, 
    constraint=FINER, end2Edges= myPart.edges.findAt(((-(dep1Width/2+(baseWidth-dep1Width)*0.5*0.1), 0, dep1Length), )), maxSize=maxMeshSizeX, minSize=minMeshSizeX)
    
myPart.seedEdgeByBias(biasMethod=SINGLE, 
    constraint=FINER, end1Edges= myPart.edges.findAt(((-(dep1Width/2+(baseWidth-dep1Width)*0.5*0.1), baseThick, dep1Length+dep2Length), )), maxSize=maxMeshSizeX, minSize=minMeshSizeX)
    
myPart.seedEdgeByBias(biasMethod=SINGLE, 
    constraint=FINER, end2Edges= myPart.edges.findAt(((-(dep1Width/2+(baseWidth-dep1Width)*0.5*0.1), 0, dep1Length+dep2Length), )), maxSize=maxMeshSizeX, minSize=minMeshSizeX)
    
myPart.seedEdgeByBias(biasMethod=SINGLE, 
    constraint=FINER, end2Edges= myPart.edges.findAt(((-(dep1Width/2+(baseWidth-dep1Width)*0.5*0.1), baseThick, baseLength), )), maxSize=maxMeshSizeX, minSize=minMeshSizeX)
    
myPart.seedEdgeByBias(biasMethod=SINGLE, 
    constraint=FINER, end1Edges= myPart.edges.findAt(((-(dep1Width/2+(baseWidth-dep1Width)*0.5*0.1), 0, baseLength), )), maxSize=maxMeshSizeX, minSize=minMeshSizeX)
...
# Tirza Peeters change 11-27-2025 commented out because this is overwritten later
'''
...
myPart.seedEdgeByBias(biasMethod=SINGLE, constraint=FINER, end1Edges=end1_edges,
                    end2Edges=end2_edges, maxSize=maxMeshSizeZ, minSize=depTrkWidth)
...
for i in range(1, int(depNoOfLyrs)):
    myPart.seedEdgeByBias(biasMethod=SINGLE, 
        ...
        maxSize=maxMeshSizeZ, minSize=minMeshSizeZ)
...
# Tirza Peeters change 11-21-2025 remove hardcoding by using the variable minMeshSizeY and maxMeshSizeYBase /// start
myPart.seedEdgeByBias(biasMethod=SINGLE, 
    constraint=FINER, end1Edges=
    myPart.edges.findAt(
    ...
    myPart.edges.findAt(
    ((-baseWidth/2, baseThick/2, baseLength), )), 
    maxSize=maxMeshSizeYBase, minSize=minMeshSizeY)
...
# Tirza Peeters change 11-21-2025 remove hardcoding by using the variable minMeshSizeY and maxMeshSizeYBase /// end

# Tirza Peeters change 11-24-2025 add meshing vertical edges deposition /// start
x_positions_dep1 = [-dep1Width/2, dep1Width/2]
z_positions_dep1 = [(baseLength - dep2Length)/2, 0]

# Deposition 1
for x in x_positions_dep1:
    for z in z_positions_dep1:
        found_edges = myPart.edges.getByBoundingBox(
            xMin=x - tol, xMax=x + tol,
            yMin=dep1_y_bot, yMax=dep1_y_top,
            zMin=z - tol, zMax=z + tol
        )
...
x_positions_dep3 = [-dep3Width/2, dep3Width/2]
z_positions_dep3 = [baseLength, baseLength- ((baseLength - dep2Length)/2)]

# Deposition 3
for x in x_positions_dep3:
    for z in z_positions_dep3:
        found_edges = myPart.edges.getByBoundingBox(
            xMin=x - tol, xMax=x + tol,
            yMin=dep3_y_bot, yMax=dep3_y_top,
            zMin=z - tol, zMax=z + tol
        )
...
for i in range(int(depNoOfLyrs)+1):
    ...
    myAssembly.Surface(name=surfName, side1Faces=
        myAssembly.instances['SubwithDep-1'].faces.findAt(((0, baseThick+subsThick+depTrkThick*i, dep1Length+dep2Length/2), )))
...
for i in range(int(depNoOfLyrs)+1):
    ...
    myAssembly.Surface(name=surfName, side1Faces=
        myAssembly.instances['BasewithDep-1'].faces.findAt(((0, baseThick+depTrkThick*i, dep1Length/2), )))
...
for i in range(int(depNoOfLyrs)+1):
    ...
    myAssembly.Surface(name=surfName, side1Faces=
        myAssembly.instances['BasewithDep-1'].faces.findAt(((0, baseThick+depTrkThick*i, dep1Length+dep2Length+dep3Length/2), )))
...
#PreHeating
# Tirza Peeters change 11-04-2025 move preheating and hotplateSS to an if-statement /// start
if Run_step_1 == 1:
...
if Run_step_2 == 1:
    ...
    if Run_step_1 == 1:
        ...
        myModel.HeatTransferStep(
            ...
            timeIncrementationMethod=FIXED, 
            initialInc=DepositionTimeInc)
# Tirza Peeters change 11-04-2025 move preheating and hotplateSS to an if-statement /// end
...
#Heat-Flux-Load
# Tirza Peeters change 11-04-2025 move preheating to an if-statement    /// start
if Run_step_2 == 1:

    for i in range(0, int(depNoOfLyrs) + 1):
...
# Tirza Peeters change 11-04-2025 move preheating to an if-statement    /// end 
...
# Tirza Peeters change 11-04-2025 move hotplateSS to an if-statement    /// start
if Run_step_1 == 1:
...
# Tirza Peeters change 11-04-2025 move hotplateSS to an if-statement    /// end
...
#########Substrate#########
# Tirza Peeters change 11-04-2025 move hotplateSS and preheatig to an if-statement  /// start
if Run_step_1 == 1:
    ...
    myModel.FilmCondition(
        ...
        sinkTemperature     =ambientTemp, 
        surface             =myAssembly.surfaces['SubstrateSides']) 
# Tirza Peeters change 11-04-2025 move hotplateSS and preheatig to an if-statement  /// end
...
# Tirza Peeters change 11-04-2025 move hotplateSS and preheating to an if-statement /// start
if Run_step_1 == 1:
    myModel.FilmCondition(
        createStepName      ='HotplateSS', 
        definition          =EMBEDDED_COEFF, 
    ...
    myModel.interactions['Clamp_Conduction'].deactivate('ReleaseFixtures')
# Tirza Peeters change 11-04-2025 move hotplateSS and preheating to an if-statement /// end
...
# Tirza Peeters change 11-04-2025 move hotplateSS to an if-statement (not sure) /// start
if Run_step_1 == 1:
    ...
    myModel.HistoryOutputRequest(
        ...
        createStepName='Deposition', 
        variables=PRESELECT)
# Tirza Peeters change 11-04-2025 move hotplateSS to an if-statement (not sure) /// end
...
C Tirza Peeters change 16-01-2026 created/renamed parameters for the dimensions of a deposition volume
      depVolThick             = {depVolThick}                   ! Thickness of each deposition volume
      depVolWidth            = {depVolWidth}                   ! Width of each deposition volume
      depVolLength            = {depVolLength}                   ! Length of each deposition volume
...
C     -------------------------------------------------------------
      ...
      xTol                  = depVolWidth/2.0d0   ! Tolerance for x-coordinate (half of deposition volume width)
      yTol                  = depVolThick/2.0d0    ! Tolerance for y-coordinate (half of deposition volume thickness)
C Tirza Peeters change 01-16-2026 changed this tolerance to half the deposition volume length instead of half the deposition volume thickness
      zTol                  = depVolLength/2.0d0   ! Tolerance for z-coordinate (half of deposition volume length)
...
C SM change 01-16-2026 added eps to deposit one deposition volume per frame
      eps = 1.0d-10*tPerFrm

C SM change 01-16-2026 changed frmTot to deposit one deposition volume per frame
      frmTot                = int((t_step + eps) / tPerFrm) ! Total number of frames, rounded to nearest integer
...
C     -------------------------------------------------------------
      ...
      tTol                  = 0.001d0
      xTol                  = depVolWidth/2.0d0
      yTol                  = depVolThick/2.0d0
C Tirza Peeters change 01-16-2026 changed this tolerance to half the deposition volume length instead of half the deposition volume thickness
      zTol                  = depVolLength/2.0d0
...
C SM change 01-16-2026 changed frmTot to deposit one deposition volume per frame
      frmTot                = int((t_raster + eps)/ tPerFrm)
...
# Tirza Peeters change 16-01-2026 added parameters depVolThick, depVolWidth and depVolLength
fortran_code = fortran_template.format(
    ...
    depVolThick="{:.3f}d0".format(depVolThick),
    depVolWidth="{:.3f}d0".format(depVolWidth),
    depVolLength="{:.3f}d0".format(depVolLength),
...
# Tirza Peeters change 11-04-2025 move hotplate and preheating to an if-statement
if Run_step_1 == 1:
...
# Tirza Peeters change 11-17-2025 change initial temperatures to the same initial temperatures as for heat transfer model   /// start
myModel.Temperature(
    createStepName          ='Initial', 
    crossSectionDistribution=CONSTANT_THROUGH_THICKNESS, 
    distributionType        =UNIFORM, 
...
myModel.Temperature(
    ...
    name                    ='Base', 
    region                  =myAssembly.instances['BasewithDep-1'].sets['Base'])  
# Tirza Peeters change 11-17-2025 change initial temperatures to the same initial temperatures as for heat transfer model   /// end
