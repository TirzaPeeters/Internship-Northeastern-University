...
% ---------------------- USER SETTINGS ---------------------------
csv_folder_paths = ["C:\Users\Tirza\Documents\Abaq_finalcleanB\", "C:\Users\Tirza\Documents\Abaq_finalcleanS\", "C:\Users\Tirza\Documents\Abaq_finalcleanXS\"];               % path to folders where csv files are stored in same order as mesh_names
mesh_names = ["B", "S", "XS"];                                                    % define names of mesh which you also used in the python file
point_names = ["P_-15_8_5_132", "P_0_8_5_152", "P_-14_8_5_133"];                  % define name of points which you also used in the python file
point_deposited = cell(length(mesh_names), 1);
point_deposited{1} = [ 39 461 39;
                       85 507 85;
                       85 507 85 ];
point_deposited{2} = [ 39 461 39;
                       85 507 85;
                       85 507 85 ];
point_deposited{3} = [ 39 461 39;
                       85 507 85;
                       85 507 85 ];                                         % frame in which node is deposited for the mesh in order NT11, S11, Smises (field output = rows, points = columns). you find this by looking at NT11 where the temperature goed from 300 to a higher value and a big increase in the stresses
...
number_of_steps = 6;
y_text = ["NT11 [K]", "\sigma_{11} [kPa]", "\sigma_{mises} [kPa]"];
% ----------------------------------------------------------------
...
for field_output = 1:length(all_fieldoutput)  
    ...
    for point = 1:length(point_names)
        ...
        % Create empty cells to store output
        T_total = cell(length(mesh_names), 1);
        time_total = cell(length(mesh_names), 1);
        time_total_deposited = cell(length(mesh_names), 1);

        for mesh = 1:length(mesh_names)
            % Load CSV data
            csv_path = csv_folder_paths(mesh) + all_fieldoutput(field_output) + "_over_time_" + mesh_names(mesh) + "_" + point_names(point) + "_nodenr_input.csv";
            ...
            % Store output and time for each mesh
            frame_deposited = point_deposited{mesh}(field_output, point);
            T_total{mesh} = T(:, frame_deposited+1:end);
            time_total{mesh} = time;
            time_total_deposited{mesh} = time(:, frame_deposited+1:end);
            ...
            % Only use values from time of deposit and further
            T = T(:, frame_deposited+1:end);
            time = time(:, frame_deposited+1:end);
        ...
        % Make figure of field output for point for all meshes
        figure;
        hold on
        for m = 1:length(mesh_names)
            plot(time_total_deposited{m}, T_total{m}, 'LineWidth', 1.5)
        end
        xline(time_total{length(mesh_names)}(frames_per_step_cumsum), '--', 'LineWidth', 1);
        hold off
        xlabel('Time (s)');
        ylabel(y_text(field_output));
        title(all_fieldoutput(field_output) + " over time for point " + P + " for different mesh sizes");
        grid on;
        legend(mesh_names, 'Location', 'best');
        %savefig("C:\Users\Tirza\Documents\My codes for postprocessing\Mesh plots top deposit\" + all_fieldoutput(field_output) + "_point_" + P + "_meshcomparison.fig");
        saveas(gcf, "C:\Users\Tirza\Documents\My codes for postprocessing\Mesh plots top deposit\" + all_fieldoutput(field_output) + "_point_" + P + "_meshcomparison.png");
